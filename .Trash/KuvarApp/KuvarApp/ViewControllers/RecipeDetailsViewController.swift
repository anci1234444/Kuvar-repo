  ...
  //  var imageData: Data?
    ...
    private func displayRecipeDetails() {
     ...
     /*   if let data = imageData {
            // If imageData is available, use it to set the image
            recipeImageView.image = UIImage(data: data)
        } else if let imageURL = recipe.imageURL, let url = URL(string: imageURL) {
            // If imageData is nil but imageURL is available, use imageURL to fetch the image
            recipeImageView.af.setImage(withURL: url)
        } else {
            // Handle the case when both imageData and imageURL are nil
            recipeImageView.image = UIImage(named: "placeholder") // Or any other default image
        }*/
